...
# Script Function: 统计被测APP的“热启动”时间，并对启动耗时进行数据可视化。
...
class AppTest(object):
    ...
    def quit_app(self):
        back_home_cmd = "adb shell input keyevent 'KEYCODE_HOME'"
        os.popen(back_home_cmd)
...
class Control(object):
    def __init__(self, count, apk_activity):
    ...
    def data_visualization(self):
        ...
        x_del_1_system_time = x_system_time[1:]

        for y in range(self.counter):                   # 提取每一次APP启动的耗时
            y_app_launch_time.append(time_list[y][1])
        y_del_1_app_launch_time = y_app_launch_time[1:]

        for z in range(self.counter-1):                   # 累加每一次APP启动耗时
            z_total_time += y_del_1_app_launch_time[z]

        average_time = int(z_total_time / (self.counter-1))     # APP启动的平均耗时
        ...
        plt.plot(x_del_1_system_time, z_average_time, 'yo')                         # 'y': 黄色; 'o': 圆标记
        plt.xticks(x_del_1_system_time, rotation=270)             # 设置x轴文本显示角度，0为水平，90为逆时针旋转90°，以此类推

        # tidy up the figure
        plt.grid(True)                                  # 添加网格线
        plt.legend(loc=0)                               # 图例，0为最佳位置，即不覆盖数据
        plt.axis('tight')                               # 使所有数据可见（缩小限值）
        plt.title('{} Total "{} Times" Hot Start App Launch Time Test Result:'.format(self.get_time_ymd(), self.counter-1))
        plt.xlabel('System Time(Hours:minutes:seconds)')
        plt.ylabel('App Launch Time Value(Milliseconds)')
        plt.savefig('{}_HotStartAppLaunchTimeTestResult.png'.format(time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime())))

if __name__ == '__main__':
    apk_activity = "com.qiyi.video/.WelcomeActivity"
    count = 31
    control = Control(count, apk_activity)
